...
links:
  ...
  pr: null
files:
  - phase-II/specs/003-frontend/spec.md
  - phase-II/specs/003-frontend/checklists/requirements.md
tests:
...
- Secure and authenticated interactions with backend APIs
- Full integration with Spec-1 (Backend) and Spec-2 (Authentication)
...
- UI displays only the authenticated user's data
- Loading, error, and empty states are handled gracefully
- Application functions correctly across desktop and mobile viewports

Constraints:
- Frontend framework is fixed: Next.js 16+ (App Router)
- API communication must strictly follow backend specifications
- All protected pages require authenticated access
- No manual coding; all code must be generated via Claude Code
- Must integrate seamlessly with Spec-1 APIs and Spec-2 authentication flow
